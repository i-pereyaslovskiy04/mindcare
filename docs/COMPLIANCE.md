...
### Просмотр журналов администратором (Stage 8, ADR-023)

- Доступ к `GET /api/admin/audit/{events,auth-events,data-changes,options}` есть
  только у активной membership-роли `admin`. Supervisor, psychologist и student
  получают 403; seed-permissions `admin:audit` / `auth:view_logs` источником
  доступа не являются
- Администратор видит **проекцию**, а не строки журнала. Физически не
  выбираются и отсутствуют в схемах ответа: `description`, сырая `metadata`,
  `ip_address`, `user_agent`, `session_id`, `mfa_method`, `request_url`,
  `request_method`, `old_values`, `new_values`
- Email всегда маскируется (`mask_email`, `i***@domain.ru`); невалидное или
  пустое значение даёт `***`. Полный адрес не возвращается ни в одном поле
- Внутренний `users.id` наружу не выходит ни в ответе, ни в query-параметрах:
  цель-человек адресуется только UUID. Попытка отфильтровать по внутреннему id
  пользователя отвергается 422
- Терапевтический контент недостижим по построению: viewer не обращается к
  `session_notes`, `chat_messages`, `test_results`, `diary_entries` и вложениям
  и не вызывает функции расшифровки
- Строка, противоречащая собственной спеке (роль вне allowlist события, чужой
  `entity_type`, незарегистрированный код отказа, поле вне allowlist таблицы,
  неизвестное событие или таблица), **редактируется**: соответствующие поля
  возвращаются пустыми с признаком `details_redacted`
- Сам просмотр журналов аудируется: событие `audit_logs_viewed` в `audit_log`
  фиксирует, кто и какой журнал открыл. В metadata попадают только имя журнала
  и стабильные ИМЕНА применённых фильтров — ни дат, ни UUID, ни идентификаторов
  записей. Если событие не удалось записать, данные журнала не отдаются (503)
- Ответы помечены `Cache-Control: no-store, private`; ETag не выставляется
- Экспорт (CSV/Excel/PDF), detail-эндпоинт, раскрытие raw IP и полного email —
  **не реализованы** и требуют отдельного product/security/DPO-решения
- Ограничение периода 90 днями — граница производительности запроса, а **не**
  политика хранения: сроки хранения строк журналов остаются открытым решением
  DPO (см. `docs/BACKLOG.md`)
