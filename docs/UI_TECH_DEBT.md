...
- **Что делать дальше:** не мигрировать в FilterChip. Shared `TimePicker` уже существует, но он выбирает произвольное время, а не backend-рассчитанный свободный слот.
- **Можно мигрировать позже:** только если появится отдельный shared `SlotPicker` для appointment-slots
- **Риск:** высокий
...
- **Почему оставлено:** декоративный dot-индикатор, не статус сущности.
- **Что делать дальше:** не мигрировать в Badge.
- **Можно мигрировать позже:** нет, не Badge
- **Риск:** низкий
- **Закрыто:** `aria-hidden="true"` добавлен
