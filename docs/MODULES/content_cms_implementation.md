...
|---|---|---|
| Потребитель | `Hero.jsx` (главная + услуги) | `ServicesSlider.jsx` → `ServiceCard.jsx` |
| Страницы | несколько, поле `placement` (`home`/`services`) | одна, поля `placement` нет |
...
миграции**. Заводить его «на будущее» в новом модуле не нужно: у карточек
услуг единственная страница, и поле было бы преждевременной абстракцией.
