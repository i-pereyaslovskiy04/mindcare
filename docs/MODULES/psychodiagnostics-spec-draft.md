...
- ⚠ **Исправлено Stage 7:** IP в `consent_records` **НЕ** анонимизируется
  `anonymize_old_ips()`. Эта функция покрывает ровно три audit-журнала
  (`audit_log`, `auth_log`, `data_change_log`); legacy-версия тоже никогда не
  затрагивала `consent_records`, так что исходное утверждение было неверным с
  самого начала. IP в `consent_records` — часть доказательства согласия и
  хранится бессрочно; политика по нему остаётся открытым вопросом DPO
  (см. `docs/BACKLOG.md`, таблица Stage 7).
