...
- **T5 — режим для слабовидящих по ГОСТ Р 52872-2019** (панель настроек: шрифт
  100/150/200%, схемы Ч/Б, Б/Ч, синяя, бежевая; интервалы; изображения) —
  токены hc уже есть, панель и правила режима — отдельный этап.
- **T1 — ручной визуальный smoke** тёмных/контрастных тем (build и контраст
  автоматически зелёные, глазами не проверялось).
- **T7** — решение по трём унаследованным парам контраста 3.0
  (`text-light`, success-badge, `coffee`-акцент) в не-hc темах.
