...
**⭐ Приоритет: экспорт результатов тестов для staff (Этап E+)**
- Сейчас `app/tests/routes_staff.py` (Этап E, ADR-016) отдаёт только
  metadata-список результатов студента и один результат по uuid
  (`GET /api/staff/test-results`, `GET /api/staff/test-results/{uuid}`) —
  никакого экспорта, агрегации по группе/студенту или сравнения динамики между
  прохождениями нет
- Студенческий CSV (на `ResultDetailPage.jsx`) — клиентский, для ОДНОГО своего
  результата; не переиспользуется staff-стороной и не покрывает bulk-выгрузку
- Нужно решить до реализации:
  - формат (CSV одного результата по аналогии со студенческим — минимальный
    шаг; агрегированный экспорт по студенту/группе — отдельная, более крупная
    задача; PDF — reportlab + новый endpoint + audit)
  - объём: один результат vs все результаты студента vs результаты по
    engagement-группе (для supervisor)
  - зона доступа наследует ADR-016: psychologist — только по active/past
    `TherapyEngagement`, supervisor — любой студент, admin — доступа к
    результатам не имеет вообще (и не должен получить его через экспорт)
  - audit: каждый экспорт — это staff-чтение содержимого результата, обязано
    писать событие по аналогии с `test_result_content_read` (Этап E) —
    вероятно, отдельное `test_result_exported`, а не переиспользование
    существующего
  - раскрытие свободного текста ответов — из охвата (см. `student_answers`
    encryption boundary в `mindcare_api/CLAUDE.md`)
- Файлы: `mindcare_api/app/tests/routes_staff.py`, `service.py`, `storage.py`;
  фронт — `mindcare_web/src/features/tests/ui/StudentTestResults.jsx`
- См. также исходное ТЗ:
  `docs/MODULES/psychodiagnostics-spec-draft.md` §1 «Вне scope MVP» —
  «Экспорт результатов для staff, сравнение динамики и графики по времени»
  (документ в целом устарел и не отражает Этапы E/F/F2/F2.1/F2.2 — не
