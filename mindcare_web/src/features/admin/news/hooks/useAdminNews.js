...
import { getAdminNews } from '../../../../api/news.api';
...
export function useAdminNews() {
  ...
  const [filters, setFilters] = useState({ is_published: null });

  const debouncedQuery = useDebounce(query, 300);
  // Монотонно растущий счётчик — только последний запрос применяет state
